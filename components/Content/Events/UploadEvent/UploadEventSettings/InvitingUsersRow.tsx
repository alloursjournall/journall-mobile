...
const InvitingUsersRow: React.FC<props> = ({ event, setEvent }) => {
      ...
      <View style={styles.slightSplitRow}>
        {event?.invited_user_id && (
          <Image
            ...
            source={returnProfileImg(event.hosting_user_id, ALL_USERS_ICONS)}
          />
        )}

        <Text style={styles.headerText}> {event?.inviting_username} </Text>
      </View>
      <Text style={styles.captionText}> invited by </Text>

      {/*  🚨 🚨 🚨 🚨 eventattendance.map() eventattendance.going = "host"  */}
    </View>
  );
};
...
export default InvitingUsersRow;
