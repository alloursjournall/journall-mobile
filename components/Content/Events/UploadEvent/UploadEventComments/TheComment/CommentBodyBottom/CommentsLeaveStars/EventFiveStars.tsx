...
const EventFiveStars: React.FC<FiveStarsProps> = ({
  comment,
  index,
  setStarClicked,
  ...
  const commentUnlockHandlerSetUserPassLocks = async (
    ...
    if (comment?.unlock?.includes("times")) {
      ...
      if (starsLength >= unlockNum) {
        ...
        // 🔒 no event locks yet:
        // const locksAfterSubmitTrue = await submitCommentThoughtALLpassCommentThoughtsTrueFunc(day?.id, comment?.id, setUsersPassLocks, setError)
      } else {
        ...
        // 🔒 no event locks yet
        // const locksAfterUpdatingFailed = await updateCommentThoughtALLpassCommentThoughtsFalseFunc(day?.id, comment?.id, setUsersPassLocks, setError)
      }
    }
    ...
    if (comment?.unlock?.includes("avg")) {
      ...
      if (starsAverage >= unlockNum) {
        // 🔒 no event locks yet
        // const locksAfterSubmitting = await submitCommentThoughtALLpassCommentThoughtsTrueFunc(day?.id, comment?.id, setUsersPassLocks, setError)
        // if (!locksAfterSubmitting) { return null; }
  ...
  const submitOneThruFiveStarsRating = async () => {
    ...
    const predata: any = await axios.post("http://localhost:4000/api/graphql", {
      query: query,
    });
