...
const MainRootCommentsLoop: React.FC<MainRootCommentsLoopProps> = ({
  //
  allUserProfileIcons,
        ...
        comments?.map((mapitem: any, index: number) => {
          ...
          const isPostingUserBlocked =
            ...
            ALL_BLOCKS?.some(
              (blocks) =>
                (blocks?.user_id === mapitem?.user_id &&
                  blocks?.blocked_id === day?.user_id) ||
                (blocks?.blocked_id === mapitem?.user_id &&
                  blocks?.user_id === day?.user_id)
            );

          return (
            (!day?.lock ||
              day?.lock !== "see comments" ||
              (day?.lock === "see comments" && seeCommentLockIsUnlocked)) &&
...
export default MainRootCommentsLoop;
